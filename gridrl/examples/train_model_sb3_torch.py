...
with warnings.catch_warnings():
    ...
    try:
        ...
        print(
            f"Couldn't load [stable_baselines3] package. Try >> pip install stable-baselines3.\n\tError: {exc}"
        )
        raise
...
if __name__ == "__main__":
    USED_NUM_ENVS = 1
    USED_FLATTEN_OBS = True
    UsedModelClass = PPO
    USED_POLICY_CLASS = "MlpPolicy" if USED_FLATTEN_OBS else "MultiInputPolicy"
    train_sb3(
        ...
        env_config={
            ...
            "flat_obs": USED_FLATTEN_OBS,
            "flat_obs_dtype": np.int16,
        },
        train_config={
            "episode_length": 2**14,
            ...
            "monitor": True,
            "normalize": False,
            "model_init_checkpoint_filename": f"{sys.path[0]}sb3_torch_{'flat' if USED_FLATTEN_OBS else 'dict'}_init_checkpoint.zip",
            "model_save_checkpoint_filename": f"{sys.path[0]}sb3_torch_{'flat' if USED_FLATTEN_OBS else 'dict'}_save_checkpoint.zip",
        },
